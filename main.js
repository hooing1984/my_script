...
var url = 'https://gitee.com/guominyang/repo1/raw/master/QuestionBank.db'; //仅用于每日答题

//var init_url = 'https://gh-proxy.com/https://raw.githubusercontent.com/mondayfirst/XXQG_TiKu/main/%E9%A2%98%E5%BA%93_%E6%8E%92%E5%BA%8F%E7%89%88.json';完整题目

//"https://gitcode.net/McMug2020/XXQG_TiKu/-/raw/master/%E9%A2%98%E5%BA%93_McMug2020.json" 截取的题目
//'https://gh-proxy.com/https://raw.githubusercontent.com/McMug2020/XXQG_TiKu/main/%E9%A2%98%E5%BA%93_McMug2020.json'与上面一样的，GitHub的备份
var init_url = 'https://gitcode.net/McMug2020/XXQG_TiKu/-/raw/master/%E9%A2%98%E5%BA%93_McMug2020.json';

var replacejs_url = 'https://gitee.com/guominyang/repo1/raw/master/replace.js';
    ...
    while (!err) {
        try {
            className("android.widget.ListView").findOnce().children().forEach(item => {
                ...
                myScores[name] = score;
              	console.log(name);console.log(score);
            });
            err = true;
        } catch (e) {
            console.log(e);
                ...
                "RDTJtCTsZ5JW+8sGvTRDzFnDeO+BcOEpP0Rte6f+HwcGxeN2dglWfgH8P0C7HkCMJOAAAAAElFTkSuQmCC").exists()) //遇到❌号，则答错了,不再通过结束本局字样判断
        {
            ...
            if (lNum >= lCount) {
                ...
                if (复活 && conNum < qCount) {
                    ...
                    textContains('立即复活').findOne().click();
                    delay(0.5);
                    //back();
                    delay(1);
                } else {
...
function do_contest_answer(depth_option, question1) {
    ...
    var pos = -1;
    var answers_list = '';
    if (rex.test(question) ||rec.test(question) || reg.test(question) || rea.test(question) || reb.test(question) || ree.test(question)) {
        音字 = true;
        first = false;
        try {
          ...
          //console.log('old_answers:' + old_answers)
            for (var k = 0; k < 2; k++) {
    ...
    if (option[0] >= 'A' && option[0] <= 'D') {
        //var ans = answer.split('    ')[option[0].charCodeAt(0) - 65];
      	var ans = answer.split('|')[option[0].charCodeAt(0) - 65];
      //console.info('option[0]：',option[0]);
      //console.info('在答案数组里的位置（从0开始）：',option[0].charCodeAt(0) - 65);
        console.info('答案:'+ans);
        var last=option;
        if(乱序 == 'a' && !first && !音字){
            while(true) {
...
function init(){
    ...
    threads.start(function () {
        try{
            ...
            var x = http.get(replacejs_url).body.string();
  ...
  /*
    ...
    */
    try{
        ...
        file_tmp = null;
        tikus = tikus.split('\n');
      // tikus[0] = 'B 蜘蛛对蛛网上不同类型振动的辨识能力是天生的。该说法是的。 正确  错误'
        for(var i = 0 ;i<tikus.length;i++){
            var t = tikus[i].split(' ');
          // t[0] = 'B'   t[1] = '蜘蛛对蛛网上不同类型振动的辨识能力是天生的。该说法是的。'   t[2] = '正确'   t[3] = '错误'
            if(t[1]&&t[0]){
                var answer = '';
        ...
        try{
          /*
            ...
            */
            ...
            let resp = http.get(init_url).body.string();
            // 转换题库格式

            // 对应的题库是 'https://gh-proxy.com/https://raw.githubusercontent.com/mondayfirst/XXQG_TiKu/main/%E9%A2%98%E5%BA%93_%E6%8E%92%E5%BA%8F%E7%89%88.json';
            /*let json = JSON.parse(resp);
            let out = "";
            for(let prop in json){
                let opt;
                let answer = json[prop];
                let arrTemp = prop.split('|');
                question = arrTemp[0]; 
                question = question.replace(/\s{1,}/g, '');  
                let answerStrs = "";
                for(let i=1; i<arrTemp.length; i++){
                    if(answer == arrTemp[i]){
                        opt = String.fromCharCode(i - 1 + 65);
                    }
                    if(i != arrTemp.length - 1){
                        answerStrs += arrTemp[i] + "\t";
                    }else{
                        answerStrs += arrTemp[i]    
                    }                    
                }

                out += opt + " " + question + " " + answerStrs + "\n";
            }*/

            // 对应的题库是 'https://gh-proxy.com/https://raw.githubusercontent.com/McMug2020/XXQG_TiKu/main/%E9%A2%98%E5%BA%93_McMug2020.json';
            resp.replace(/\\"/g,"“"); 
            let json = JSON.parse(resp);
            let out = "";
            for(let prop in json){
            ...
            //console.info(tikus.slice(0, 200));
            //console.info(tikus.slice(tikus.length-200, tikus.length));
            //is.close();
            file_tmp = true;
            //hamibot.exit();
            //exit();
